  ...
  def archiveUser(self, **event_args):
    ...
    if self.user["enabled"]:
      ...
      if confirm(
        ...
        employmentType = confirm(
          "How are they employed?", buttons=["Full Time", "Part Time", "Contractor"]
        )
        if employmentType:
          anvil.server.call(
            "archiveUser", self.item["employee"]["UserID"], False, employmentType
          )
          self._parent.sortFilteredEmployees()  # Refreshes the entire repeating panel
